...
面向个人交易者的交易辅助系统。产品提供市场总览、新闻影响、规则决策、Hyperliquid/Binance/OKX 只读账户监控、模拟交易和 AI 复盘，不包含真实自动下单能力。
...
移动端原型包含 iPhone / Pixel 10 预览壳、单币种 K 线、规则决策、币种切换、五个底部导航页面和决策依据弹层。市场快照与 `1m / 5m / 1h / 4h / 1d` K 线读取交易所公共接口；决策、最多三个计划持仓、动态管理建议、真实完成交易和复盘由服务端持久化。

模拟交易按平台独立保存，默认净值为 1000 USDC。净值只用于累计盈亏与复盘，不限制模拟开仓资金；最多同时执行三项可交易决策，并按设置的决策周期扫描、按计划止盈止损自动完成模拟结算。Cloudflare Worker 每分钟持续处理启用的钱包，App 关闭后仍会按已完成的 1 分钟 K 线推进。钱包余额、持仓、历史和审计事件均以 D1 服务端记录为准；K 线断档时暂停结算而不猜测触价顺序。模拟执行不会调用任何交易所下单接口，也不会写入真实策略训练样本。
...
行情通过三家平台的公共接口获取。Hyperliquid 使用公开钱包地址；Binance/OKX 使用用户自行创建的只读 API 凭证，后端仅调用账户、持仓和成交历史 GET 接口，不包含下单或撤单调用。决策必须依次通过 EMA20/50/200 排列与斜率、均线间距、价格与 EMA50 结构、RSI 极值、MACD 与 RSI 动量一致性、成交量、资金费率拥挤度及价格偏离 EMA20 的追价检查；任一硬门槛失败均返回 WAIT 且仓位为零。通过准入后优先使用 ATR 生成 0.25–0.75 ATR 回踩入场区、距入场中位价 1.25 ATR 的止损以及 2R/3R 止盈目标，ATR 缺失时兼容原固定百分比模型。完成交易时必须同时找到决策开始后的真实开仓与平仓成交；退出价按成交量加权，净盈亏按交易所已实现盈亏减去 USDT/USDC 手续费计算。其他手续费币种在尚未换算前会拒绝生成错误复盘。AI 不能修改任何成交事实；AI 未配置、拒绝或调用失败时，复盘仅保留事实并标记 `analysis_engine: facts`，不会使用规则分析或规则调参代替。

后端会二次验证 AI 调参方案：五维权重必须合计 100，单项权重和 `min_trade_score` 必须在安全范围内，单次变化不得超过限制。验证通过后才保存新的 `strategy_versions` 记录；不合格方案会被标记为 `rejected`。新决策会读取当前已验证策略版本，扫描缓存也按策略版本隔离，避免版本升级后继续返回旧评分。每个平台最近最多 200 笔去重模拟决策会形成独立的历史策略：满 12 笔后可把准入门槛调整 -2～+4 分并把风险预算缩放至 0.5～1.05 倍，单方向满 8 笔后再叠加方向表现；连续亏损会优先提高门槛并缩仓。历史策略不改写五维评分，也不会在缺少 MFE/MAE 路径样本时调整入场、止盈或止损。
