...
后端会二次验证 AI 调参方案：五维权重必须合计 100，单项权重和 `min_trade_score` 必须在安全范围内，单次变化不得超过限制。验证通过后才保存新的 `strategy_versions` 记录；不合格方案会被标记为 `rejected`。当前五维评分仍固定使用 `v1` 参数，真实交易生成的 AI 调参版本不会自动生效。每个平台最近最多 200 笔去重模拟决策会形成独立的历史策略：满 12 笔后可把准入门槛调整 -2～+4 分并把风险预算缩放至 0.5～1.05 倍，单方向满 8 笔后再叠加方向表现；连续亏损会优先提高门槛并缩仓。历史策略不改写五维评分，也不会在缺少 MFE/MAE 路径样本时调整入场、止盈或止损。
