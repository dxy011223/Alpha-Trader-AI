...
行情通过三家平台的公共接口获取。Hyperliquid 使用公开钱包地址；Binance/OKX 使用用户自行创建的只读 API 凭证，后端仅调用账户、持仓和成交历史 GET 接口，不包含下单或撤单调用。完成交易时必须同时找到决策开始后的真实开仓与平仓成交；退出价按成交量加权，净盈亏按交易所已实现盈亏减去 USDT/USDC 手续费计算。其他手续费币种在尚未换算前会拒绝生成错误复盘。配置 AI 后，模型只负责解释这些已核验指标并给出改进建议，不能修改成交事实；调用失败或达到 Redis 配额时保留规则复盘并标记 `analysis_engine: rules`。
...
新闻来自 CoinDesk、CNBC Markets 与 Federal Reserve 的公开 RSS，并按真实发布时间归档；新闻源不可用时不会生成假新闻。配置 `AI_API_KEY` 后，决策接口通过 OpenAI Responses API 和结构化输出生成评分，再由本地风险公式计算入场、止损、止盈与仓位；未配置或调用失败时会明确返回 `analysis_engine: rules`。所有密钥均通过环境变量配置，禁止硬编码。
